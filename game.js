...
const bird={
  ...
  update: function() {
    // if the game state is get ready state flap slower otherwise faster
    this.period = state.current==state.getReady ? 10 : 5;
    this.frame += frames % this.period==0 ? 1 : 0;
    this.frame = this.frame % this.animation.length;
    
    if(state.current==state.getReady){
      this.y=150; //reset the positon for new game
      ...
      //if the speed is greater than the jump bird going down
...
const pipes = {
  ...
  update: function(){
    ...
    for(let i=0; i<this.position.length; i++)
    {
      ...
      //if pipes go beyond the left of canvas
      if(p.x + this.w<=0){
        this.position.shift();
        score.value+=1;
        SCORE_S.play();
